...
class CrossSections:
    def __init__(self, energy, lepton):
        if energy <= 11:
            ...
            E_v = energy
            mom_e = m_e**2 + 2*m_e*E_v
            mom_u = m_u**2 + 2*m_u*E_v
            kinematic_e = mom_e**2    # mass in GeV c-2
            kinematic_u = mom_u**2
            
            sigma_naught_e = (G_f**2 * kinematic_e) / np.pi
            sigma_naught_u = (G_f**2 * kinematic_u) / np.pi
            print("Sigma Naught E", sigma_naught_e)
            if lepton == 'e':
            ...
            for flavour in self.cs_without_ms.keys():
                ...
                self.cs_without_ms[flavour].append(cs * energy * sigma_naught)
    ...
    def gate_cs(self):
        ...
        single_reactions = ['e_e', 'e_u', 'u_e', 'u_u']
        single_reactions_anti = ['E_E', 'E_U', 'U_U']
        gate_reactions = []
        for reaction in single_reactions:   #split into neutrino and anti neutrino
...
combine_cs(leptons['e'], leptons['u'])
plt.show()
